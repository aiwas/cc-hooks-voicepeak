...
### 3. hook が終了コード 1 で落ちる経路がある — `cli.py:400`
...
### 4. デタッチ起動でボイス指定が黙って捨てられる — `hook.py:90`
...
### 5. 設定ファイル由来の値が型変換されない — `config.py:289`
...
### 6. インラインコード内のスネークケースが壊れる — `normalize.py:184`
...
### 7. パスでない文字列がパス扱いされ前半が消える — `normalize.py:34`
